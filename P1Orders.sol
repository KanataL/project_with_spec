    ...
    P1TraderConstants
{
     ...
     */
       /*@
        ensures _getOrderHash(order) == \old(_getOrderHash(order));
        reverts_when msg.sender != order.maker;
        ensures _STATUS_[_getOrderHash(order)] == OrderStatus.Approved || STATUS_[_getOrderHash(order)] == OrderStatus.Canceled;
    */
     ...
     */
     /*@
        ensures _getOrderHash(order) == \old(_getOrderHash(order));
        reverts_when msg.sender != order.maker;
        ensures _STATUS_[_getOrderHash(order)] == OrderStatus.Canceled;
    */
