    ...
    P1Storage
{
     ...
     */
    /*@
        ensures block.timestamp >= \old(_GLOBAL_INDEX.timestamp);
        ensures _GLOBAL_INDEX.timestamp >= \old(_GLOBAL_INDEX.timestamp);
        ensures _ORACLE == \old(_ORACLE);
        ensures _FUNDER == \old(_FUNDER_);
    ...
    */
     ...
     */
    /*@
        ensures context.price == \old(context.price);
        ensures context.minCollateral == \old(context.minCollateral);
        ensures context.index.timestamp >= \old(_LOCAL_INDEXES_[account].timestamp);
        ensures context.index.isPositive == \old(context.index.isPositive);
        ensures context.index.value == \old(context.index.value);
        ensures _LOCAL_INDEXES_[account].timestamp >= \old(_LOCAL_INDEXES_[account].timestamp);
        ensures \old(_BALANCES_[account].position) == _BALANCES_[account].position;
        ensures \old(_BALANCES_[account].positionIsPositive) == _BALANCES_[account].positionIsPositive;
    */
    ...
    /*@
        ensures context.price == \old(context.price);
        ensures context.minCollateral == \old(context.minCollateral);
        ensures context.index.timestamp == \old(context.index.timestamp);
        ensures context.index.isPositive == \old(context.index.isPositive);
        ensures context.index.value == \old(context.index.value);

        ensures balance.marginIsPositive == \old(balance.marginIsPositive);
        ensures balance.positionIsPositive == \old(balance.positionIsPositive);
        ensures balance.margin == \old(balance.margin);
        ensures balance.position == \old(balance.position);
    */
